...
const AdminOrderPage = () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ orderId, status, note }) => {
      const res = await api.put(`/orders/update-status/${orderId}`, {
        status,
        note,
      });
      return res.data;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-7 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="text-2xl font-bold">
              {formatPriceBDT(stats.totalRevenue)}
            </div>
          </div>
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {orderList.map((order) => (
                  <tr key={order._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(order.orderStatus)}`}
                      >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getPaymentStatusColor(order.payment?.status)}`}
                      >
              ...
              <div className="p-6">
                ...
                <div className="mb-6 p-4 bg-gray-50 rounded-lg flex justify-between items-center flex-wrap gap-4">
                  <div>
                    ...
                    <span
                      className={`ml-2 px-2 py-1 text-sm font-semibold rounded-full ${getStatusColor(selectedOrder.orderStatus)}`}
                    >
                  ...
                  <div>
                    ...
                    <span
                      className={`ml-2 px-2 py-1 text-sm font-semibold rounded-full ${getPaymentStatusColor(selectedOrder.payment?.status)}`}
                    >
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {selectedOrder.shippingAddress?.area || "N/A"}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Post Code</p>
                      <p className="font-medium">
                        {selectedOrder.shippingAddress?.postCode || "N/A"}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {selectedOrder.shippingArea || "N/A"}
                      </p>
                    </div>
                  </div>
                </div>
